...
class CargaMasivaService {
    ...
    async procesarCargaMasiva(archivos, userId, opciones = {}) {
        const {
            ...
            loteId = null,
            origen = "DIRECTO",
            allowSinNomenclatura = false,             // ← NUEVO parámetro importante
        ...
        try {
            if (useOcr) {
                // Modo OCR → por ahora sigue siendo estricto (puedes adaptarlo después si quieres)
                for (const archivo of archivos) {
                    try {
                        const datosArchivo = this.parsearNombreArchivo(archivo.nombre);
                        const autorizacionInfo = await this.buscarOCrearAutorizacion(datosArchivo, userId);
                ...
                // Modo SIN OCR → aquí sí respetamos allowSinNomenclatura
                for (let i = 0; i < archivos.length; i += loteSize) {
                    const lote = archivos.slice(i, i + loteSize);
                    const promesas = lote.map(async (archivo) => {
                        let proceso = null;
                        ...
                        try {
                            const nombre = archivo.nombre || 'sin-nombre.pdf';

                            if (allowSinNomenclatura) {
                                datosArchivo = {
                                    numeroAutorizacion: `TMP-${Date.now().toString().slice(-8)}-${Math.floor(Math.random() * 1000).toString().padStart(3, '0')}`,
                                ...
                                datosArchivo = this.parsearNombreArchivo(nombre);
                            }
                            ...
                            proceso = await this.ocrProcesoModel.create({
                                ...
                                metadata: {
                                    ...
                                    modo: allowSinNomenclatura ? 'sin-nomenclatura' : 'normal'
                                },
                            });

                            const resultado = await this.procesarArchivoMasivo(
                                { ...archivo, nombreOriginal: datosArchivo.nombreOriginal || nombre },
                                autorizacionInfo,
                                userId
                            );
                            ...
                            resultados.exitosos++;
                            resultados.detalles.push({ archivo: nombre, exito: true, ...resultado });
                            ...
                            resultados.fallidos++;
                            resultados.detalles.push({
                                archivo: archivo.nombre || 'sin-nombre',
                                exito: false,
                                error: error.message
                            });
                        }
     ...
     */
    async iniciarProcesamientoOCRAsincrono(archivoBuffer, extension, userId, loteId, opciones = {}) {
        const {
            ...
            for (const archivo of archivos) {
                ...
                try {
                    ...
                    const proceso = await this.ocrProcesoModel.create({
                        ...
                        nombre_archivo: nombre,               // ← CORREGIDO
                        autorizacion_id: autorizacionInfo.autorizacion.id,
                        user_id: userId,
                        estado: "pendiente",
                        tipo_proceso: "OCR_ASINCRONO",
                        origen: origen,
                        metadata: {
                            datosArchivo,
                            tamano: archivo.tamano,
                            ...
                            modo: allowSinNomenclatura ? 'sin-nomenclatura' : 'normal',
                            fallbackUsado: allowSinNomenclatura
                        },
                    });
                    ...
                    this.enviarArchivoParaOCR(archivo, proceso, autorizacionInfo, userId)
                        .catch(err => {
                            console.error(`Error enviando ${nombre} a OCR:`, err);
                            proceso.update({
                                estado: 'fallado',
                                error: err.message
                            }).catch(console.error);
                    ...
                    await this.ocrProcesoModel.create({
                        ...
                        origen: origen,
                        error: error.message,
                        metadata: { error: true, modo: allowSinNomenclatura ? 'sin-nomenclatura' : 'normal' }
                    });
                }
            }
    ...
    /**
     * Iniciar procesamiento directo OCR asíncrono (para PDFs subidos uno a uno o múltiples)
     */
    async iniciarProcesamientoDirectoOCRAsincrono(archivos, userId, loteId, opciones = {}) {
        ...
        for (const archivo of archivos) {
            const nombre = archivo.originalname || archivo.nombre || 'sin-nombre.pdf';
            let datosArchivo;
            let autorizacionInfo;
            ...
            try {
                ...
                const proceso = await this.ocrProcesoModel.create({
                    lote_id: loteId,
                    archivo_id: `init_${Date.now()}_${Math.random().toString(36).slice(2, 10)}`,
                    nombre_archivo: nombre,
                    autorizacion_id: autorizacionInfo.autorizacion.id,
                    user_id: userId,
                    estado: "pendiente",
                    ...
                    metadata: {
                        modo: allowSinNomenclatura ? 'sin-nomenclatura' : 'normal',
                        fallback: allowSinNomenclatura,
                        datosUsados: datosArchivo,
                ...
                this.enviarArchivoParaOCR(
                    { buffer: archivo.buffer, nombre, tamano: archivo.tamano || archivo.size },
                    proceso,
                    autorizacionInfo,
                    userId
                ).catch(err => {
                    console.error(`Error background envío OCR ${nombre}:`, err);
                    proceso.update({ estado: 'fallado', error: err.message }).catch(console.error);
                });

            } catch (err) {
                console.error(`Error preparando archivo ${nombre}:`, err.message);

                await this.ocrProcesoModel.create({
                    lote_id: loteId,
                    archivo_id: `err_${Date.now()}`,
                    nombre_archivo: nombre,
                    autorizacion_id: autorizacionInfo?.autorizacion?.id ?? null,
                    user_id: userId,
                    estado: "fallado",
                    ...
                    origen,
                    error: err.message,
                    metadata: { error: true, intentoSinNomenclatura: allowSinNomenclatura }
                });
            }
        }
        ...
        return {
            loteId,
            total: archivos.length,
            iniciados: procesos.length,
            modo: allowSinNomenclatura ? 'sin-nomenclatura' : 'estricto'
        };
    }
